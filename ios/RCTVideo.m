...
{
  NSLog(@"VideoPlayer - setSrc: %@", source);
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
    ...
    NSLog(@"VideoPlayer - onVideoLoadStart");
...
- (AVPlayerItem*)playerItemForSource:(NSDictionary *)source
{
  ...
  NSLog(@"VideoPlayer - player item for source is network %d isAsset: %d", isNetwork, isAsset);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
  if (object == _playerItem) {
    ...
    if ([keyPath isEqualToString:statusKeyPath]) {
      ...
      _playerBufferEmpty = YES;
      NSLog(@"VideoPlayer - isBuffering YES");
      ...
      NSLog(@"VideoPlayer - isBuffering NO");
    ...
    if([keyPath isEqualToString:readyForDisplayKeyPath] && [change objectForKey:NSKeyValueChangeNewKey]) {
      if([change objectForKey:NSKeyValueChangeNewKey]){
        NSLog(@"VideoPlayer - onReadyForDisplay");
...
{
  NSLog(@"VideoPlayer - playbackStalled ");
...
{
  NSLog(@"VideoPlayer - setSeek %.2f", seekTime);
  int timeScale = 10000;
  ...
  if (item && item.status == AVPlayerItemStatusReadyToPlay) {
    ...
    if (CMTimeCompare(current, cmSeekTime) != 0) {
      if (!_paused) {
        NSLog(@"VideoPlayer - seek compare, was not paused - PAUSE");
        // Pause during seek
        [_player pause];
      }
...
{
  NSLog(@"VideoPlayer - setRate %.2f", rate);
  _rate = rate;
  [self applyModifiers];
}
...
{
  NSLog(@"VideoPlayer - setVolume %.2f", volume);
  _volume = volume;
  [self applyModifiers];
}
...
- (void)applyModifiers
{
  NSLog(@"VideoPlayer - applyModifiers %.2f, paused: %d", _volume, _paused);
  if (_muted) {
    [_player setVolume:0];
